...
verfügbare_woche = 3360 if vorgabe == "Voll (56h)" else int(st.number_input("Eigene Eingabe (h)", 0.0, 56.0, 36.0, 0.25) * 60)
...
if st.button("➕ Zwischenstopp hinzufügen") and len(st.session_state.zwischenstopps) < 10:
...
if pause_aktiv:
    abfahrt_datum = st.date_input("📅 Datum", now_local.date())
...
with col_a:
    ...
    zehner_fahrten = [st.checkbox(f"✅ 10h-Fahrt Nr. {i+1}", value=True) for i in range(2)]
with col_b:
    st.subheader("9h-Ruhepausen (max. 3)")
    neuner_ruhen = [st.checkbox(f"✅ 9h-Ruhepause Nr. {i+1}", value=True) for i in range(3)]

# Wochenruhepause
wochenruhe_manuell = st.checkbox("🛌 Wochenruhepause während Tour einfügen?")
if wochenruhe_manuell:
    we_tag = st.date_input("Start der Wochenruhe", now_local.date(), key="we_date")
    ...
    if data["status"] != "OK":
        ...
        while remaining > 0:
            if we_start and we_start <= current_time < we_ende:
                ...
                zehner_index = neuner_index = 0
                log.append(f"🛌 Wochenruhe: {we_start.strftime('%d.%m %H:%M')} – {we_ende.strftime('%d.%m %H:%M')}")
                continue

            if current_time.weekday() == 0 and current_time.hour >= 2:
                zehner_index = neuner_index = 0
                log.append("🔄 Wochen-Reset (Montag ab 2:00)")
            ...
            log.append(f"📆 {current_time.strftime('%A %H:%M')} → {gefahren} min + {pausen} min → Ende: {ende.strftime('%H:%M')}")
            remaining -= gefahren
            ...
            log.append(f"🌙 Ruhe {ruhezeit//60}h → weiter: {(ende + timedelta(minutes=ruhezeit)).strftime('%d.%m %H:%M')}")
        ...
        if verbleibend_min < 0:
            h_m, m_m = divmod(abs(verbleibend_min), 60)
            st.warning(f"⚠️ Wochenlenkzeit überschritten um {h_m}h {m_m}min")
        else:
        ...
        ziel_tz = pytz.timezone(get_timezone_for_address(ziel_für_zeit))
