...
if we_aktiv:
    ...
    we_ende = we_beginn + timedelta(hours=we_dauer_std)
else:
    we_beginn = None
    we_ende = None
...
if st.button("📦 Route analysieren & ETA berechnen"):
    ...
    if data["status"] != "OK":
        ...
        while remaining_minutes > 0:
            if we_aktiv and we_beginn <= current_time <= we_ende:
                current_time = we_ende
                log.append(f"🛌 Wochenruhezeit {we_dauer_std}h – Weiterfahrt ab: {current_time.strftime('%Y-%m-%d %H:%M')}")
                # Zurücksetzen der 9h-Pausen nach WE
                used_9h_rest = 0
                neuner_index = 0
                continue
