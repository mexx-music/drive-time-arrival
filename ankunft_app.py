...
startort = st.text_input("📍 Startort", "Lambach, Österreich")
zielort = st.text_input("🏁 Zielort", "Bukarest, Rumänien")
...
if pause_aktiv:
    ...
    abfahrt_uhrzeit = st.time_input("🕓 Uhrzeit der Abfahrt", value=dt_time(4, 0))
    ...
    abfahrtsuhrzeit = st.time_input("Uhrzeit", value=now_local.time().replace(second=0, microsecond=0))
    abfahrtszeit = datetime.combine(abfahrtsdatum, abfahrtsuhrzeit)
...
zehner_fahrten = [st.checkbox(f"10h-Fahrt nutzen (Tag {i+1})", value=True, key=f"10h_{i}") for i in range(2)]

st.subheader("🌙 9-Stunden-Ruhepausen (max. 3/Woche)")
neuner_ruhen = [st.checkbox(f"9h-Ruhe erlaubt (Nacht {i+1})", value=True, key=f"9h_{i}") for i in range(3)]

tankpause = st.checkbox("⛽ Zusätzliche Tankpause einplanen (30 min)")

st.subheader("🛌 Wochenruhezeit")
we_aktiv = st.checkbox("🛏️ Wochenendpause aktivieren")
we_dauer = st.selectbox("Dauer der Wochenruhezeit", ["24h", "45h", "66h"])
we_startdatum = st.date_input("📅 Startdatum der Wochenruhezeit", value=now_local.date() + timedelta(days=2))
we_startzeit = st.time_input("🕓 Startzeit der Wochenruhezeit", value=dt_time(12, 0))
...
if st.button("📦 Route analysieren & ETA berechnen"):
    ...
    if data["status"] != "OK":
        ...
        if we_aktiv:
            we_begin = local_tz.localize(datetime.combine(we_startdatum, we_startzeit))
            we_ende = we_begin + timedelta(hours=int(we_dauer[:-1]))
        else:
            we_begin = we_ende = None

        while remaining_minutes > 0:
            tag = current_time.strftime("%A")
            start_str = current_time.strftime("%Y-%m-%d %H:%M")

            if we_aktiv and we_begin <= current_time <= we_ende:
                log.append(f"🛌 Wochenruhezeit: {we_dauer} → weiter ab {we_ende.strftime('%Y-%m-%d %H:%M')}")
            ...
            if remaining_minutes > 0:
                if neuner_index < len(neuner_ruhen) and neuner_ruhen[neuner_index]:
                    ...
                    rest = 660
                    ruhetyp = "11h-Ruhe"
                log.append(f"🌙 Ruhezeit: {ruhetyp} → weiter ab {(tages_ende + timedelta(minutes=rest)).strftime('%Y-%m-%d %H:%M')}")
