...
st.title("🚛 DriverRoute ETA – inkl. Lenkzeit & Karte")
...
if st.button("📦 Route analysieren & ETA berechnen"):
    ...
    if data["status"] != "OK":
        ...
        def zeige_google_karte(orte_liste):
            base_url = "https://maps.googleapis.com/maps/api/staticmap?"
            size = "640x400"
            markers = "&".join([f"markers=color:red%7C{urllib.parse.quote(o)}" for o in orte_liste])
            path = f"path=color:blue|weight:5|{urllib.parse.quote('|'.join(orte_liste))}"
