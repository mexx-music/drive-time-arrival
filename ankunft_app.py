...
if st.button("📦 Route analysieren & ETA berechnen"):
    ...
    if data["status"] != "OK":
        ...
        # Falls keine Dauer von Google: auf Basis km/h schätzen
        ...
        while rest > 0:
            ...
            # Tagesruhe
            ruhezeit = 660  # 11h = 660min
            if verbleibende_9h > 0:
                ruhezeit = 540  # 9h
                verbleibende_9h -= 1
            ankunft += timedelta(minutes=ruhezeit)

            # Wochenendpause einbauen falls Zeit erreicht
            if we_aktiv and ankunft >= we_beginn and ankunft < we_ende:
                diff = we_ende - ankunft
                ankunft += diff  # auf Ende Wochenruhezeit springen

        st.success(f"🛣️ Strecke: {round(gesamt_distanz_km)} km")
        st.success(f"⏱️ Fahrzeit (Google): {gesamt_dauer_min} Minuten")
        st.success(f"📅 ETA (unter Berücksichtigung aller Regeln): **{ankunft.strftime('%A, %d.%m.%Y – %H:%M')} Uhr**")
