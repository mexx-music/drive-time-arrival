...
if st.button("📦 Berechnen & ETA anzeigen"):
    ...
    if data["status"] != "OK":
        ...
        # Zielzeitzone anzeigen
        ziel_tz = pytz.timezone(get_timezone_for_address(zielort))
        ende_zielzeit = ende.astimezone(ziel_tz)
        de_zeit = ende.strftime('%A, %d.%m.%Y – %H:%M')
        ziel_zeit = ende_zielzeit.strftime('%H:%M')
        ziel_zone = ziel_tz.zone

        st.markdown(f"""
        <h2 style='text-align: center; color: green;'>
        ✅ <u>Ankunftszeit:</u><br>
        🕓 <b>{de_zeit}</b> <span style='color: gray;'>(Zielzeit: {ziel_zeit} – {ziel_zone})</span>
        </h2>
